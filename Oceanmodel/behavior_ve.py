...
#def behavior_move_ocean(timestep: int, oceanholders: dict, veaccounts: dict, minlock: int, maxlock: int) -> Dict[uuid.UUID, list]:
    '''
    ...
    if timestep == 20 or timestep == 120 and valid_locking_agents:
        action_lock = 'lock'
        selected_locking_agent = random.choice(valid_locking_agents)
        #print(f"~timestep: {timestep}")
        #print(f"~valid locking agent: {valid_locking_agents} | selected locking agent: {selected_locking_agent}")
    ...
    valid_withdrawing_agents = [key for key in oceanholders.keys() if [acct for acct in oceanholders[key].veaccounts.keys() if oceanholders[key].veaccounts[acct].unlocked > 0] and key not in selected_locking_agent]
    # Logic determines which agents withdraw their unlocked ocean. EXCLUDING any agent who is locking ocean.
    if False: #timestep % 25 == 0 and valid_withdrawing_agents:
        ...
        #print(f"~valid withdrawing agents: {valid_withdrawing_agents}")# | selected withdrawing agent: {selected_withdrawing_agent}")
        #for i in range(len(oceanholders[selected_withdrawing_agent].veaccounts)):
        #    if veaccounts[oceanholders[selected_withdrawing_agent].veaccounts[i]].unlocked >= 0:
        #        valid_veaccount.append(oceanholders)
        #
        #valid_veaccount = [oceanholders[selected_withdrawing_agent].veaccounts[i] for i in range(len(oceanholders[selected_withdrawing_agent].veaccounts)) if veaccounts[oceanholders[selected_withdrawing_agent].veaccounts[i]].unlocked >= 0]
        valid_veaccount = [acct for acct in oceanholders[selected_withdrawing_agent].veaccounts.keys() if oceanholders[selected_withdrawing_agent].veaccounts[acct].unlocked > 0]
        selected_veaccount = random.choice(valid_veaccount)
        #print(f"~valid veaccount: {valid_veaccount} | selected veaccount: {selected_veaccount}")
        #unlocked_balance = veaccounts[selected_veaccount].unlocked
        unlocked_balance = oceanholders[selected_withdrawing_agent].veaccounts[selected_veaccount].unlocked
    ...
    acting_agents[selected_withdrawing_agent] = (action_withdraw, withdraw_amount, withdraw_duration, selected_veaccount)
